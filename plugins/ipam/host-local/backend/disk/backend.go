...
func (s *Store) HasReservedIP(podNS, podName string) (bool, net.IP) {
	...
	// Pod, ip mapping info are recorded with file name: ip_PodIP_PodNS_PodName
	...
	if len(podFileName) != 0 {
		ipStr, ns, name := resolvePodFileName(podFileName)
...
// edge k8s: ReservePodInfo create podName file for storing or updating ip
func (s *Store) ReservePodInfo(ip net.IP, podNS, podName string) error {
	if len(podName) == 0 {
		return nil
	}
	...
	if ip == nil {
		...
		if len(podFileName) > 0 {
			podIPNsNameFilePath := GetEscapedPath(s.dataDir, podFileName)
			err = ioutil.WriteFile(podIPNsNameFilePath, []byte(deletedPodInfo), 0644)
			if err != nil {
				return err
			}
		}
	...
	// for adding pod, create a new file named "ip_PodIP_PodNS_PodName",
	// if there is already file named with "ip_*_PodNS_PodName", rename the old file with new PodIP.
	targetPodFileName := podFileName(ip.String(), podNS, podName)
	targetPodFilePath := GetEscapedPath(s.dataDir, targetPodFileName)
	foundPodFileName, err := s.findPodFileName("", podNS, podName)
	if err != nil {
		return err
	}

	if len(foundPodFileName) != 0 && targetPodFileName != foundPodFileName {
		oldPodFilePath := GetEscapedPath(s.dataDir, foundPodFileName)
		err = os.Rename(oldPodFilePath, targetPodFilePath)
		if err != nil {
			return err
		}
	}

	err = ioutil.WriteFile(targetPodFilePath, []byte{}, 0644)
	if err != nil {
		return err
	}
...
// podFileName ip_PodIP_PodNS_PodName
...
func (s *Store) RemoveExpiredRecords(pattern string, expirationDays int) error {
	...
	err := filepath.Walk(s.dataDir, func(path string, info os.FileInfo, err error) error {
		...
		if strings.TrimSpace(string(data)) == deletedPodInfo {
			if err := os.Remove(path); err != nil {
				return nil
			}
		}
