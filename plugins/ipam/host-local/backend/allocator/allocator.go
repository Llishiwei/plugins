...
func (a *IPAllocator) GetByPodNsAndName(id string, requestedIP net.IP, podNs, podName, ifname string) (*current.IPConfig, error) {
	a.store.Lock()
	...
	if len(podName) != 0 {
		...
		if podIPIsExist {
			// podName file is exist, update ip file with new container id and ifname.
			_, err := a.store.ReservePodInfo(id, ifname, knownIP, podNs, podName, podIPIsExist)
			if err != nil {
				return nil, err
			}
			...
			if ipCfg != nil {
				_, err := a.store.ReservePodInfo(id, ifname, ipCfg.Address.IP, podNs, podName, podIPIsExist)
				if err != nil {
					return ipCfg, err
				}
			}
